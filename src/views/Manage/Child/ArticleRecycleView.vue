    ...
    >
      ...
      <el-table-column label="删除时间" min-width="90">
        <template #default="scope">
          <span>{{ formatDateTime(scope.row.markDelTime) }}</span>
        </template>
      </el-table-column>
...
import { queryArticle, deleteArticle, restoreArticle } from "api/article";
    ...
    getData() {
      ...
      queryArticle(this.params)
        .then((res) => {
          ...
          list = list.sort((a, b) => new Date(a.markDelTime) - new Date(b.markDelTime));
          this.data = list;
          ...
          for (let i = 0; i < this.data.length; i++) {
            ...
            let date = new Date(this.data[i].markDelTime);
