...
export default {
  ...
  methods: {
    ...
    openEdit(index) {
      const arr = this.data;
      this.category = JSON.parse(JSON.stringify(arr[index]))
      this.show = true;
      this.isEdit = true;
    },
    closeEdit() {
    ...
    openAdd() {
      this.show = true;
      this.isEdit = false;
      this.category = JSON.parse(JSON.stringify(this.init))
    },
    add() {
      this.saveLoading = true;
      Category.add(this.category.name)
        .then((res) => {
          this.saveLoading = false;
          if (res.content) {
            this.$message.success("操作成功");
    ...
    check() {
      if (!this.category.name || String(this.category.name).trim().length === 0) {
        return;
      }
      this.inputLoading = true;
      Category.check(this.category.name).then((res) => {
